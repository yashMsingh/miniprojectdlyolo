...
class AssignmentReportGenerator:
    """Generate professional assignment reports."""
    ...
    def generate_full_report(
        ...
        output_path: str,
        stats: Dict[str, Any],
        detections_log: List[Dict[str, Any]] | None = None,
    ) -> bool:
        """Generate complete assignment report.

        Args:
            output_path: Where to save report.
            stats: Statistics dictionary from runtime.
            detections_log: Optional detections log.
        ...
        try:
            ...
            report = self._build_report(stats, detections_log)

            with open(output_path, "w", encoding="utf-8") as f:
                f.write(report)

            if self.logger:
                self.logger.info(f"Assignment report generated: {output_path}")
            return True
        except Exception as exc:
            if self.logger:
                self.logger.exception(f"Report generation failed: {exc}")
            return False
    ...
    def _build_report(
        ...
        report = f"""# YOLOv8 Real-Time Object Detection - Assignment Report
...
"""
    ...
    def _format_statistics(self, stats: Dict[str, Any]) -> str:
        ...
        stats_items = [
            ...
            ("Average Detections/Frame", f"{stats.get('avg_objects_per_frame', 0):.2f}"),
            ("Average FPS", f"{stats.get('avg_fps', 0):.2f}"),
            ("Average Inference Time", f"{stats.get('avg_inference_ms', 0):.2f} ms"),
            ("Session Duration", f"{stats.get('duration_seconds', 0):.1f} sec"),
        ]
        ...
        if "class_counts" in stats and stats["class_counts"]:
            ...
            for cls, count in sorted(counts.items(), key=lambda x: x[1], reverse=True)[:10]:
